...
/* Matches the `phone` breakpoint in globals.css from the other side, so the
   one place that has to branch in JS branches at the same width the rest of
   the page branches in CSS. */
const narrow = mediaStore("(max-width: 560px)");
...
/** True only where a hover-capable, precise pointer is driving. */
export function useFinePointer(): boolean {
  return useSyncExternalStore(finePointer.subscribe, finePointer.get, onServer);
}

/** Below the `phone` breakpoint. The statement sentence uses it to drop its
    inline clips to poster frames: at that width the line is already breaking
    three ways and running video inside it is noise rather than evidence.
    Answers false on the server, so the wide layout is what gets prerendered
    and a phone corrects itself in the same pass that reads the motion
    preference. */
