...
import { TEXT_H1, TEXT_LEAD, TEXT_META } from "@/lib/redesign/ui";
import { ReelWallV6, type ReelColumns } from "@/components/sections/ReelWallV6";
...
/* THE HEADLINE POPS IN WORD BY WORD, the way auto-captions land in the ads.
   WORD_START is when the first word pops, WORD_STEP the gap between words, and
   HEADLINE_MS roughly when the last one has settled, which is when the lines
   under it start to fade up. */
const HEADLINE_WORDS = hero.headline.split(" ").length;
const WORD_START = 150;
const WORD_STEP = 80;
const HEADLINE_MS = WORD_START + (HEADLINE_WORDS - 1) * WORD_STEP;
...
function useHeroFade() {
  ...
  return (i: number) => ({
    style: phase === "in" ? { transitionDelay: `${HEADLINE_MS + i * 90}ms` } : undefined,
    className:
      phase === "armed"
...
export function HeroV6({
  ...
  const [sub, ctas, reassure] = [fade(1), fade(2), fade(3)];
  ...
  return (
    ...
    >
      ...
      <div
        className={`${STAGE} grid items-center ${SPLIT_GAP} lap:grid-cols-[46fr_54fr]`}
      >
        ...
        <div className="flex flex-col items-center text-center lap:items-start lap:text-left">
          {/* The headline, set as on-screen ad captions that pop in word by
              word on load — see components/ui/CaptionText. */}
          <h1
            className={`max-w-[16ch] px-[0.2em] text-balance font-display ${TEXT_H1} font-extrabold leading-[1.2]`}
          >
            <CaptionText text={hero.headline} pop={{ start: WORD_START, step: WORD_STEP }} />
          </h1>
          ...
          >
            ...
            <a
              ...
              className="inline-flex min-h-[44px] items-center px-4 font-sans font-bold text-ink underline decoration-ink/30 underline-offset-[0.3em] transition-colors hover:decoration-ink"
            >
              {hero.secondaryCta}
            </a>
          </div>
          ...
          <p
            ...
            className={`mt-4 font-sans ${TEXT_META} text-ink-faint ${reassure.className}`}
          >
            {hero.reassurance}
          </p>
        </div>
        ...
        <div className="min-w-0">
          ...
          <p
            className={`mt-[16px] text-center font-sans ${TEXT_META} text-ink-faint lap:translate-x-[calc(50%_-_(1.9*(clamp(116px,12vw,158px)_+_2*clamp(4px,0.6vw,6px))_+_20px))]`}
          >
