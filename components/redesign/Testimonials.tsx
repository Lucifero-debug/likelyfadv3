...
const BADGE =
  `pointer-events-none absolute right-3 top-3 rounded-full bg-white/92 px-2.5 py-1 font-sans ${TEXT_META} font-medium leading-none text-ink backdrop-blur-[2px]`;
...
function Card({
  ...
  /** True once the row has scrolled into view; the message then lands. */
  arrived: boolean;
  /** ms after arrival before THIS card's message replaces its typing dots. */
  arriveDelay: number;
}) {
      ...
      <figcaption className="pt-5">
        ...
        {reel && (
          /* TRACKING COMES OFF ON A PHONE AND NOTHING ELSE DOES. This line is
             ...
             carry it. */
          <p className={`font-sans ${TEXT_META} text-ink-faint`}>
            Sent about this ad
          </p>
        )}

        {/* THE QUOTE IS SET AS THE MESSAGE IT WAS: these came in as DMs, so
            they read as a received message bubble rather than as a pull quote
            in curly marks. Never clamped — a cut-off client quote reads as
            edited.

            AND IT ARRIVES LIKE ONE. Until the row scrolls into view each card
            shows a "typing" bubble; on arrival the dots give way to the message
            after this card's delay, so the row fills in like a thread. The quote
            is always in the DOM and only faded, so the layout never shifts and
            a screen reader reads it from the start. */}
        <div className={`${reel ? "mt-2" : ""} relative w-fit max-w-full`}>
          <blockquote
            style={{ transitionDelay: arrived ? `${arriveDelay}ms` : "0ms" }}
            className={`origin-top-left rounded-[18px] rounded-tl-[6px] bg-[#efedf0] px-4 py-2.5 text-pretty font-sans text-[clamp(1rem,0.95rem+0.25vw,1.125rem)] leading-snug text-ink transition-[opacity,scale] duration-300 ease-[cubic-bezier(0.2,0.8,0.2,1)] ${
              arrived ? "scale-100 opacity-100" : "scale-95 opacity-0"
            }`}
          >
            {item.quote}
          </blockquote>
          <span
            aria-hidden
            style={{ transitionDelay: arrived ? `${arriveDelay}ms` : "0ms" }}
            className={`absolute left-0 top-0 flex items-center gap-1 rounded-[18px] rounded-tl-[6px] bg-[#efedf0] px-4 py-[0.95rem] transition-opacity duration-150 ${
          ...
          >
            {[0, 1, 2].map((d) => (
              <span
                key={d}
                className="typing-dot size-1.5 rounded-full bg-ink-soft"
        ...
        {/* No avatar and no name: these clients asked to stay unnamed, so the
            attribution is the role and the moment, in plain text. */}
        <p className={`mt-2 pl-1 font-sans ${TEXT_META} text-ink-faint`}>{item.who}</p>
      </figcaption>
    </figure>
  );
}
...
export function Testimonials() {
  ...
  /* THE MESSAGES ARRIVE ONCE, when a third of the row is on screen. Under
     reduced motion every delay is zero, so the quotes are simply there. */
  ...
  return (
    ...
    <section className={SECTION} aria-label={testimonials.kicker}>
      <div className={WRAP}>
        ...
        <h2
          className={`${HEAD_GAP} max-w-[12ch] ${HEADING}`}
        >
          ...
          >
            {testimonials.items.map((t, i) => (
              /* h-full so a card whose quote runs to three lines does not leave
                 ...
                 staggered against. */
              <div key={t.quote} className={ITEM}>
                <Card
                  item={t}
                  ...
                  arrived={arrived}
                  arriveDelay={instant ? 0 : 700 + Math.min(i, 4) * 550}
                />
              </div>
            ))}
          </div>
